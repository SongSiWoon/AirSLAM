...
RosPublisher::RosPublisher(const RosPublisherConfig& ros_publisher_config): Node("ros_publisher_node"), _config(ros_publisher_config){
...
//      _ros_feature_pub.publish(ros_feature_msg);
  ...
  if(_config.frame_pose){
    ...
    _pub_latest_odometry = this->create_publisher<nav_msgs::msg::Odometry>(_config.frame_odometry_topic, 1000);
...
//      _ros_frame_pose_pub.publish(pose_stamped);
      ...
      odometry.header.stamp = rclcpp::Time(frame_pose_message->time);
...
//      br.sendTransform(tf::StampedTransform(transform, pose_stamped.header.stamp, "map", "camera"));
  ...
  if(_config.keyframe){
    ...
    _ros_keyframe_array.header.stamp = this->get_clock()->now();
    ...
    _ros_path.header.stamp = this->get_clock()->now();
    ...
    std::function<void(const KeyframeMessageConstPtr&)> publish_keyframe_function =
      ...
      _ros_keyframe_array.header.stamp = rclcpp::Time(keyframe_message->time);
      _ros_path.header.stamp = rclcpp::Time(keyframe_message->time);
  ...
  if(_config.map){
    // for mappoints
    _ros_map_pub = this->create_publisher<sensor_msgs::msg::PointCloud>(_config.map_topic, 1);
    //_ros_map_pub = nh.advertise<sensor_msgs::PointCloud> (_config.map_topic, 1);
    _ros_mappoints.header.stamp = this->get_clock()->now();
    ...
    std::function<void(const MapMessageConstPtr&)> publish_map_function =
      ...
      _ros_mappoints.header.stamp = rclcpp::Time(map_message->time);
    ...
    _ros_maplines.header.stamp = this->get_clock()->now();
    ...
    std::function<void(const MapLineMessageConstPtr&)> publish_mapline_function =
      ...
      _ros_maplines.header.stamp = rclcpp::Time(mapline_message->time);
  ...
  if(_config.reloc){
    ...
    _ros_reloc_traj_pub = this->create_publisher<visualization_msgs::msg::Marker>(_config.reloc_topic+"/trajectory", 1);
    //_ros_reloc_traj_pub = nh.advertise<visualization_msgs::Marker>(_config.reloc_topic+"/trajectory", 1);
    _ros_reloc_traj.header.stamp = this->get_clock()->now();
    ...
    // for current pose
    _ros_reloc_pose_pub = this->create_publisher<geometry_msgs::msg::PoseStamped>(_config.reloc_topic+"/pose", 10);
    ...
    std::function<void(const RelocMessageConstPtr&)> publish_reloc_pose_function =
      ...
      geometry_msgs::msg::PoseStamped pose_stamped;
      pose_stamped.header.stamp = rclcpp::Time(reloc_message->times[idx]);
    ...
    // for matches
    _ros_reloc_mpts_pub = this->create_publisher<visualization_msgs::msg::Marker>(_config.reloc_topic+"/matches", 1);
    ...
    std::function<void(const RelocMessageConstPtr&)> publish_reloc_mpts_function =
      ...
      ros_reloc_mpts.header.stamp = rclcpp::Time(reloc_message->times[idx]);
